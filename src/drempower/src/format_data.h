...
static inline void uint8_to_data(uint32_t val, uint8_t *data)
{
    data[3] = (uint8_t)(val >> 24);
    data[2] = (uint8_t)(val >> 16);
    data[1] = (uint8_t)(val >> 8);
...
static inline uint32_t data_to_uint8(uint8_t *data)
{
		uint32_t tmp_uint;
		tmp_uint = (((uint32_t)data[3] << 24) + ((uint32_t)data[2] << 16) + ((uint32_t)data[1] << 8)  + ((uint32_t)data[0]));
		return tmp_uint;
...
static inline void int8_to_data(int val, uint8_t *data)
{
    data[0] = *(((uint8_t*)(&val)) + 0);
    data[1] = *(((uint8_t*)(&val)) + 1);
    data[2] = *(((uint8_t*)(&val)) + 2);
...
static inline int data_to_int8(uint8_t *data)
{
	int tmp_int;
    *(((uint8_t*)(&tmp_int)) + 0) = data[0];
    *(((uint8_t*)(&tmp_int)) + 1) = data[1];
...
{
    int span = (1 << bits) - 1;
    int offset = x_max - x_min;
    if(x > span)
        x = span;
    else if (x < 0)
        x = 0;
    return (float)(offset * x / float(span) + x_min);
}
